#!/bin/bash
# NAS transmission 폴더 자동 정리 (lftp로 삭제)
# Usage: ./auto_cleanup_lftp.sh
...
# 현재 사용률 확인 (lftp로 du -s 실행)
get_usage() {
    lftp -c "open -u $NAS_USER,$NAS_PASS ftp://$NAS_HOST; df -h" | grep /volume1 | awk '{print $5}' | tr -d '%'
}
...
while true; do
    ...
    # 4GB+ 파일 찾기 (lftp로 큰 파일 하나)
    NEXT_FILE=$(lftp -c "open -u $NAS_USER,$NAS_PASS ftp://$NAS_HOST; find $TARGET_PATH -type f -size +${MIN_SIZE_MB}M -exec du -h {} + 2>/dev/null | sort -hr | head -1")
    ...
    # 삭제 실행 (lftp)
    lftp -c "open -u $NAS_USER,$NAS_PASS ftp://$NAS_HOST; rm '$FILE_PATH'"
...
echo ""
echo "volume1 현재 상태:"
lftp -c "open -u $NAS_USER,$NAS_PASS ftp://$NAS_HOST; df -h"