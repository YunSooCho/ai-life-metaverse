...
- [ ] 상점 메뉴 버그 수정 (Issue #153, 2026-02-24 발견) - 상점 목록이 표시되지 않음 🚨
- [ ] 월드맵 시스템 UI 구현 (Issue #133) - 낮은 우선순위
- [ ] 진화 시스템 UI 구현 (Issue #134) - 낮은 우선순위
- [ ] 타이틀 시스템 UI 구현 (Issue #135) - 낮은 우선순위
- [ ] E2E 브라우저 테스트 자동화 (Issue #59)
- [ ] 남은 시나리오 테스트 (S06, S08, S10~S14)
...
- ✅ Phase 12: 캐릭터 시스템 고급화 - Issue #113 (2026-02-19 ~ 2026-02-20)
  - EvolutionManager: 진화 시스템 (단계/스타일/조건/외형 변화)
    - 진화 단계 6단계 (기본 ~ 최종 진화)
    - 진화 스타일 4종류 (전사/마법사/레인저/서포터)
    - 오라 효과 5종류 (반짝임 ~ 신성)
  - SkillManager: 스킬 시스템 (액티브/패시브)
    - 기본 스킬 9종류 (전투3/이동3/보조3)
    - 스킬 장착/해제/레벨업/쿨타임 관리
    - 액티브 효과 (버프/디버프)
  - EquipmentSystem: 장비 시스템 (슬롯별 착용)
    - 장비 슬롯 5종류 (무기/머리/몸통/장신구/특수)
    - 장비 레어도 5단계 (Common ~ Mythic)
    - 장비 강화 시스템
  - Socket.io 이벤트 핸들러 18개 통합
